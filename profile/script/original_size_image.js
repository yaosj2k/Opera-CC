...
/*------------------------------设置-------------------------------*/
	...
	//是否显示半透明黑色背景
	//true——显示；false——不显示
	var TRANSLUCENT_BACKGROUND = true;
...
/*---------------------------设置结束---------------------------------*/
	...
	function create_preview(x, y, imgLink){
		if(imgLink !== null){
			...
			preview_div.setAttribute('style', 'border: 1px solid gray !important; line-height: 0px !important; background-color: ' + PREVIEW_DIV_COLOR + ' !important; position: absolute !important; z-index: 9999 !important; padding: ' + PREVIEW_DIV_PADDING + 'px !important;');
			...
			preview_img.setAttribute('style', 'margin-top: 0px !important; cursor: move !important; max-width: none !important; max-height: none !important; width: auto !important; height: auto !important;');
			if(SHOW_TOOLS_BAR){
				preview_img.style.marginTop = '3px';
			}
			window.addEventListener('mousedown', drag_begin, false);
			window.addEventListener('mouseup', drag_over, false);
	...
	function create_preview_loading(){
		...
		preview_loading.setAttribute('style', 'border: 1px solid gray !important; background-color: white !important; position: absolute !important; z-index: 9999 !important; width: 100px !important; height: 105px !important;');
	...
	function destroy_preview(){
		...
		if(preview_div !== null){
			window.removeEventListener('mousedown', drag_begin, false);
			window.removeEventListener('mousemove', drag_proc, false);
			window.removeEventListener('mouseup', drag_over, false);
	...
	function show_view_icon(imgLink, resized){
		...
		icon_view.addEventListener('click', function(){
			if(window !== window.top){
		...
		if( can_click_direct_view() ){
			...
			target_obj.addEventListener('click', function(ev){
				...
				if(preview_div == null || link != preview_img.src){
					if(window !== window.top){
						var message = target_x + '+|+|+' + target_y + '+|+|+' + link + '+|+|+' + 1 + '+|+|+' + 'MSG_ID_ORIGINAL_SIZE_IMAGE_CREATE';
						destroy_view_icon();
						_Message(message);
					}else{
	...
	function closePreview(ev){
		...
		if(!find_preview && !mouse_lock)
			destroy_preview();
		if(!find_loading && !mouse_lock)
			destroy_preview_loading();
		mouse_lock = false;
	}
