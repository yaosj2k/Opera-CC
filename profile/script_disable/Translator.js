...
window.addEventListener('message', function(e){
	switch(e.data){
		case 'Dict_loaded':{
			var unfind=document.evaluate('//body/div[@class="mut_jies"]',document,null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null).snapshotItem(0);
			if (unfind){
	...
	///////////////////
		case 'zdic_loaded':{
			var unfind=document.body.innerHTML.indexOf('请输入汉字')!=-1;
			if(unfind){
		...
		case 'zdic_collapse' :{
			...
			document.getElementById('N_t_zlogo_d').innerHTML=D.style.display=='none'? '＞':'∨';
		};break;
		case 'zdic_expand' :{
			var z=document.getElementById('N_t_z_iframe');
			z.style.display='';
...
function N_Dict_request(s_text){
	...
	N_t_set.iframe.addEventListener('load',function(){
		N_t_set.iframe.contentWindow.postMessage('Dict_loaded')
	},false);

/*
	//无法跨域操纵DOM
	N_t_set.iframe.addEventListener('load',
		function(){
			alert(N_t_set.iframe.contentWindow);
			alert(N_t_set.iframe.contentDocument);
	},false);
*/
};
...
function N_zdic_request(s_text){
	...
	N_t_set.ziframe.addEventListener('load',function(){
		N_t_set.ziframe.contentWindow.postMessage('zdic_loaded');
	},false);
};
